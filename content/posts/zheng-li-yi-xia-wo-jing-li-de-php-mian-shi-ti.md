...
1、用 css、html 编写一个两列布局的网页，右侧固定宽度 200px，左侧自适应。（答案如下，IE 下没测试，Chrome 下正常）
...
> 二者区别只有一个,那就是对包含文件的需求程度, include 就是包含,如果被包含的 文件不存在的话,那么则会提示一个错误,但是程序会继续执行下去。而 require 意思是需要,如果被包含文件不存在或者无法打开的时候,则会提示错误, 并且会终止程序的执行。这两种结构除了在如何处理失败之外完全一样 。once 的意思是一次,那么 include_once 和 require_once 表示只包含一次,避免重复包含。
...
> 1. 尽量选择较小的列 2) 将 where 中用的比较频繁的字段建立索引 3) select 子句中避免使用‘\*’ 4) 避免在索引列上使用计算 、not in 和<>等操作 5) 当只需要一行数据的时候使用 limit 1 6) 保证单表数据不超过 200W,适时分割表。 针对查询较慢的语句,可以使用 explain 来分析该语句具体的执行情况。
