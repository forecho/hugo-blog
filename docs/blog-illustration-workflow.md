...
- **纯观点小节（引言、结论、叮嘱型的「几个坑」）** → **干脆不放图**，连装饰插画也别放，开篇只留封面。这类内容是论述，配图只会打断阅读。
- 判断标准：这一节有没有「可以结构化成表格/卡片/箭头」的内容？有就做信息图，没有就纯文字。

**图文分工去重（核心原则，别忽略）：** 信息图承载了数据后，正文**不要再把图里的数字/清单复述一遍**——读者读两遍会觉得啰嗦，整篇显得「很乱、无法阅读」。让图负责速览数据，正文只留判断、取舍和图里没有的提醒。写法上：先 `Read` 本地 PNG 看图里到底画了哪些数字/标的/参数，正文里这些一律删掉，只保留「给谁用、什么坑、我的取舍」。

> 反面教材：Bitget 那篇第一版每个 H2 都「先一张信息图、正文再念一遍图里的数字」，被反馈「很乱」，于是删掉引言/坑/结论 3 张装饰图、四节正文对照信息图去重，才清爽。

不必每个 H2 都配图；只有「能结构化成图」的小节才配，纯观点节留白。
...
- **图文重复 = 很乱**：信息图出完，正文务必对照图把重复的数字/清单删掉（见第 0 节「图文分工」），否则读者读两遍，整篇显得乱。
- **未来日期文章线上看不到**：CI（`.github/workflows/deploy.yml`）用 `hugo --minify`，**不带 `--buildFuture`**，发布时间晚于当前时刻的文章会被生产构建跳过。本地 `--buildFuture` 能看、线上却 404 就是这个原因。`date` 别设成未来（除非故意定时，且知道要等 cron 或下次 push 在该时间之后才会上线）。
- **线上 URL 是 `xxx.html` 不是 `xxx/`**：`config.toml` 里 `posts = "/:filename"`，Hugo 生成的是 `<slug>.html` 文件，不是目录。验证/分享用 `https://blog.forecho.com/<slug>.html`，带斜杠的目录式 URL 会 404。
